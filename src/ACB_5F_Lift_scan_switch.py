...
# Constants
DOOR_POLYGON = [(-5.28, -6.92), (-2.27, -6.81), (-2.45, -0.64), (-5.75, -0.66)]

class ACB5FLiftScanSwitcher:
    def __init__(self):
        rospy.init_node('ACB_5F_Lift_scan_switch', anonymous=True)
        ...
        # Publisher
        self.scan_pub = rospy.Publisher('/ACB_5F_Lift/scan', LaserScan, queue_size=10)
    ...
    def publish_scan(self):
        if not self.current_pose or not self.lidar_top_scan or not self.livox_back_scan:
            rospy.logwarn("Missing pose or scan data, cannot publish /ACB_5F_Lift/scan") # Use logwarn
            return

        if self.is_in_door_area():
            self.scan_pub.publish(self.livox_back_scan)
            rospy.loginfo("Publishing /livox_back/scan to /ACB_5F_Lift/scan") # Add log info
        else:
            self.scan_pub.publish(self.lidar_top_scan)
            rospy.loginfo("Publishing /lidarTop/scan to /ACB_5F_Lift/scan") # Add log info
...
if __name__ == '__main__':
    try:
        switcher = ACB5FLiftScanSwitcher()
        rospy.spin()
