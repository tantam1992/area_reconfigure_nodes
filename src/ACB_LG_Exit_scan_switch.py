...
# Constants
DOOR_POLYGON = [(-37.51, -9.14), (-38.03, -6.11), (-32.92, -5.45), (-32.58, -8.49)]

class ACBLGExitScanSwitcher:
    def __init__(self):
        rospy.init_node('ACB_LG_Exit_scan_switch', anonymous=True)
        ...
        # Publisher
        self.scan_pub = rospy.Publisher('/ACB_LG_Exit/scan', LaserScan, queue_size=10)
        ...
        rospy.loginfo("ACB_LG_Exit scan switch node started") # Add a start message
    ...
    def publish_scan(self):
        if not self.current_pose or not self.lidar_top_scan or not self.livox_back_scan:
            rospy.logwarn("Missing pose or scan data, cannot publish /ACB_LG_Exit/scan") # Use logwarn
            return

        if self.is_in_door_area():
            self.scan_pub.publish(self.livox_back_scan)
            rospy.loginfo("Publishing /livox_back/scan to /ACB_LG_Exit/scan") # Add log info
        else:
            self.scan_pub.publish(self.lidar_top_scan)
            rospy.loginfo("Publishing /lidarTop/scan to /ACB_LG_Exit/scan") # Add log info
...
if __name__ == '__main__':
    try:
        switcher = ACBLGExitScanSwitcher()
        rospy.spin()
